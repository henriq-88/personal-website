import {
  SiteMessage, SiteError, SnackbarMessage,
} from '@/types'
import {
...
class SnackbarModule extends VuexModule {
  ...
  addSiteMessage ({
    clear = false, message = ``, type = ``, code = ``, snackMessage = ``,
  }: SiteMessage) {
    ...
    const newMessage = {
      message, type, code, snackMessage,
    }
    if (clear) {
      this.setSiteMessagesMutation([
        newMessage,
      ])
  ...
  addSiteError ({
    err, snackMessage,
  }: { err: SiteError, snackMessage: TranslateResult }) {
    console.error(err)
    if (err.snackMessage) {
